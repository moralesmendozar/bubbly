        ...
        for year in years:
            ...
            for col_name in column_names:
                ...
                if dataset_by_year[col_name].size != 0:
                    grid = pd.concat([grid, {'value': list(dataset_by_year[col_name]), 'key': temp}], ignore_index=True)
                    #grid = grid.append({'value': list(dataset_by_year[col_name]), 'key': temp}, ignore_index=True)
    else:
        # Check if this can be simplified
        for col_name in column_names:
            # Each column name is unique
            grid = pd.concat([grid, {'value': list(dataset_by_year[col_name]), 'key': col_name + '_grid'}], ignore_index=True)
            ...
            for category in categories:
                ...
                for col_name in column_names:
                    ...
                    if dataset_by_year_and_cat[col_name].size != 0:
                        grid = pd.concat([grid, {'value': list(dataset_by_year_and_cat[col_name]), 'key': temp}], ignore_index=True)
        ...
        for category in categories:
            ...
            for col_name in column_names:
                ...
                if dataset_by_cat[col_name].size != 0:
                        grid = pd.concat([grid, {'value':list(dataset_by_cat[col_name]), 'key': temp}], ignore_index=True)
